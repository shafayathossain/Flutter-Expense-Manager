...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  PublishSubject<List<EntryWithCategoryAndWallet>> topFiveEntries =
      PublishSubject();
  ...
  Stream<HomeState> mapEventToState(HomeEvent event) async* {
    if (event is GetWalletsEvent) {
      ...
      if (_balanceEvent != null) {
        ...
        yield* _getExpensesOfAllCategories(
            _balanceEvent.startTime, _balanceEvent.endTime);
        yield* _getTopFiveEntries(
            _balanceEvent.startTime, _balanceEvent.endTime);
      }
  ...
  Stream<HomeState> _getCashFlow(int startTime, int endTime) async* {
    List<CashFlowOfDay> event =
        await _repository.getCashFlow(startTime, endTime);
    ...
    while (startDateTime.millisecondsSinceEpoch <= endTime) {
      CashFlowOfDay result = event.firstWhere(
          (element) => element.date == startDateTime.millisecondsSinceEpoch,
          orElse: () =>
              (CashFlowOfDay(0.0, 0.0, startDateTime.millisecondsSinceEpoch)));
  ...
  Stream<HomeState> _getExpensesOfAllCategories(
      int startTime, int endTime) async* {
    List<ExpenseOfCategory> result =
        await _repository.getTotalExpenseForAllCategories(startTime, endTime);
    if (result != null) {
  ...
  Stream<HomeState> _adjustWalletBalance(
      double amount, WalletWithBalance wallet) async* {
    double adjustedBalance = amount - wallet.balance;
    int date = DateTime(DateTime.now().year, DateTime.now().month,
            DateTime.now().day, 0, 0, 0, 0, 0)
        .millisecondsSinceEpoch;
    final result = await _repository.adjustWalletBalance(
        adjustedBalance, date, wallet.mWallet.id);
    yield* _getWalletsWithBalance();
  }

  Stream<HomeState> _createWallet(String name, int color) async* {
    final result = await _repository.createWallet(name, color);
    yield* _getWalletsWithBalance();
  }
}
