...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  Stream<HomeState> _getWalletsWithBalance() async* {
    _repository.getWalletsWithBalance().map((event) {
      wallets.sink.add(event.map((e) {
        double percent =
            e.income > 0.0 ? (e.balance.abs() / e.income.abs()) : 0;
        print("PERCENT $percent");
        e..balancePercent = percent;
      }).toList());
    });
    yield HomeState();
  }
  ...
  Stream<HomeState> _getCashFlow(int startTime, int endTime) async* {
    double income = 0;
    double expense = 0;
    _repository.getCashFlow(startTime, endTime).map((event) {
    ...
    yield CashFlowState(income, expense);
  }
}
