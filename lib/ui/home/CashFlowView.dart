...
class CashFlowState extends State<CashFlowView> {
  ...
  void _showDatePickerDialog() async {
    ...
    if(pickedDates != null) {
      ...
      if(pickedDates.length == 2) {
        ...
        final dateString = "${formatter.format(pickedDates[0])} to ${formatter.format(pickedDates[1])}";
        setState(() {
  ...
  void _scrollToPosition(int index) {
    final position =_dateRangeListViewKey.currentContext.size.width * index / 5;
    _scrollController.animateTo(position, curve: Curves.ease, duration: Duration(microseconds: 1000));
  }
}